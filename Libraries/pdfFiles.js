...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Set the default active card (desktop: index 1, mobile: index 0)
    function setDefaultActiveCard() {
        const defaultIndex = window.innerWidth < 991 ? 0 : 1;
        if (defaultIndex >= 0 && defaultIndex < items.length) {
            activateCard(items[defaultIndex]);
        }
    }

    // Activate a specific card and deactivate others
    function activateCard(card) {
        ...
        items.forEach((item) => {
            ...
            // Pause videos for non-active cards and save their current time
        ...
        // Add active class and play video for the active card
        card.classList.add("active");
        const video = card.querySelector("video");
        if (video) {
            // Resume video from the last paused time if available
            if (videoStates.has(card)) {
                video.currentTime = videoStates.get(card);
            }
            try {
                video.play();
            } catch (error) {
    ...
    // Handle card activation when in viewport (80%)
    const cardObserverCallback = (entries) => {
        entries.forEach((entry) => {
            const card = entry.target;
            if (entry.isIntersecting) {
                activateCard(card); // Activate the card in the viewport
            }
        });
    };

    // Initialize the IntersectionObserver (only for screens < 991px)
    function initializeCardObserver() {
        if (cardObserver) {
            cardObserver.disconnect(); // Destroy existing observer
        }

        if (window.innerWidth < 991) {
            cardObserver = new IntersectionObserver(cardObserverCallback, observerOptions);
            items.forEach((item) => cardObserver.observe(item)); // Attach observer to all cards
        }
    }

    // Lazy load videos only when the parent component is in the viewport
    const parentObserverCallback = (entries) => {
        entries.forEach((entry) => {
            if (entry.isIntersecting) {
    ...
    const parentObserver = new IntersectionObserver(parentObserverCallback, {
        root: null, // Viewport as the root
        rootMargin: "0px",
        threshold: 0.8, // Trigger when 80% of the parent component is visible
    });

    parentObserver.observe(parentComponent);

    // Add hover functionality (only for desktops)
    function initializeHoverListeners() {
        if (window.innerWidth >= 991) {
            items.forEach((item) => {
                item.addEventListener("mouseenter", function () {
                    activateCard(item); // Activate the hovered card
                });

                item.addEventListener("mouseleave", function () {
                    setDefaultActiveCard(); // Reset to default card when hover ends
                });
            });
        }
    }
